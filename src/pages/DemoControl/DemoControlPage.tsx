...
export function DemoControlPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Competition simulator" title="Demo Control" description="Jalankan cerita operasional deterministik. Semua halaman membaca perubahan dari repository yang sama." actions={<Button variant={store.presentationMode ? "primary" : "secondary"} leadingIcon={<MonitorUp size={17} />} onClick={store.togglePresentationMode}>{store.presentationMode ? "Keluar Presentation" : "Presentation Mode"}</Button>} />
      ...
      <Card className="p-5 sm:p-6">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div><h2 className="font-semibold">Simulasi Koneksi Aplikasi</h2><p className="mt-1 text-sm leading-6 text-foreground-muted">Mengontrol perilaku data dan sync dalam demo. Ini tidak memutus koneksi browser atau sensor secara nyata.</p></div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant={connectivity.demoOverride === "degraded" ? "primary" : "secondary"} leadingIcon={<WifiLow size={16} />} onClick={() => { connectivity.setDemoOverride("degraded"); void syncPendingMutations(); }}>Connection Degraded</Button>
      ...
      <p className="rounded-xl border border-border bg-surface p-4 text-xs leading-5 text-foreground-muted">Simulator memperbarui repository mock dengan interval per langkah, bukan setiap frame. Reset mengembalikan Kolam B ke Risk Score 22, menghapus alert simulasi, dan mempertahankan navigasi aplikasi.</p>
      <Dialog open={resetOpen} onOpenChange={setResetOpen} title="Reset seluruh data demo?" description={`${connectivity.pendingCount ? `Ada ${connectivity.pendingCount} perubahan yang belum tersinkron. ` : ""}Sensor, alert, rekomendasi, action log, outbox, dan status simulator akan dikembalikan ke kondisi awal.`}>

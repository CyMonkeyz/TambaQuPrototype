...
export function ReportsPage() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Tinjauan operasional"
        title="Laporan Tambak"
        description="Lihat perubahan kualitas air, tingkat risiko, kondisi perangkat, dan tindak lanjut dalam satu ringkasan."
        ...
        {isLoading ? <LoadingSkeleton rows={5} /> : error || !data || !report ? <ErrorState onRetry={retry} /> : (
          <div className="space-y-5">
            <section className="grid grid-cols-2 gap-3 lg:grid-cols-6" aria-label="KPI laporan">
              ...
              <ReportMetric icon={CheckCircle2} label="Tindakan" value={report.kpis.actionsTaken} detail="Sudah dicatat" />
              <ReportMetric icon={RadioTower} label="Waktu Aktif" value={`${report.kpis.uptime}%`} detail="Perkiraan data demo" />
            ...
            <section className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
              <Card className="overflow-hidden">
                <div className="border-b border-border p-5"><h2 className="font-semibold">Ringkasan Kualitas Air</h2><p className="mt-1 text-xs text-foreground-muted">Nilai gabungan sesuai kolam dan periode terpilih.</p></div>
